 ...
 * </details>
 */
export function dynamicClientRegistrationClientRegistrationDeleteApiForm(
  client: AuthleteCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.ClientRegistrationDeleteApiFormRequest$outboundSchema.parse(
        value,
      ),
  ...
  const body = Object.entries(payload.ClientRegistration2Request || {}).map(
    ([k, v]) => {
  ...
  const path = pathToFunc("/api/{serviceId}/client/registration/delete")(
    pathParams,
  );
  ...
  const context = {
    ...
    operationID: "client_registration_delete_api_form",
    oAuth2Scopes: [],
