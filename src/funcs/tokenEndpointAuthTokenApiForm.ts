 ...
 * </details>
 */
export function tokenEndpointAuthTokenApiForm(
  client: AuthleteCore,
  request: operations.AuthTokenApiFormRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.AuthTokenApiFormResponse,
    | AuthleteError
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.AuthTokenApiFormRequest$outboundSchema.parse(value),
  ...
  const body = Object.entries(payload.APILBraceserviceIdRBraceAuthToken || {})
    .map(([k, v]) => {
  ...
  const path = pathToFunc("/api/{serviceId}/auth/token")(pathParams);
  ...
  const context = {
    ...
    operationID: "auth_token_api_form",
    oAuth2Scopes: [],
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.AuthTokenApiFormResponse$inboundSchema),
