 ...
 * API.
 */
export function clientManagementClientSecretUpdateApiForm(
  client: AuthleteCore,
  request: operations.ClientSecretUpdateApiFormRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.ClientSecretUpdateApiFormResponse,
    | AuthleteError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.ClientSecretUpdateApiFormRequest$outboundSchema.parse(value),
  ...
  const body = Object.entries(payload.ClientSecretUpdateRequest || {}).map(
    ([k, v]) => {
  ...
  const context = {
    ...
    operationID: "client_secret_update_api_form",
    oAuth2Scopes: [],
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.ClientSecretUpdateApiFormResponse$inboundSchema),
