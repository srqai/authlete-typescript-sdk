 ...
 * </details>
 */
export function authorizationEndpointAuthAuthorizationIssueApiForm(
  client: AuthleteCore,
  request: operations.AuthAuthorizationIssueApiFormRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.AuthAuthorizationIssueApiFormResponse,
    | AuthleteError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.AuthAuthorizationIssueApiFormRequest$outboundSchema.parse(
        value,
      ),
  ...
  const body = Object.entries(payload.AuthorizationIssueRequest || {}).map(
    ([k, v]) => {
  ...
  const path = pathToFunc("/api/{serviceId}/auth/authorization/issue")(
    pathParams,
  );
  ...
  const context = {
    ...
    operationID: "auth_authorization_issue_api_form",
    oAuth2Scopes: [],
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.AuthAuthorizationIssueApiFormResponse$inboundSchema),
