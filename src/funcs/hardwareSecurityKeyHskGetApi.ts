 ...
 * Get Security Key
 */
export function hardwareSecurityKeyHskGetApi(
  client: AuthleteCore,
  request: operations.HskGetApiRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.HskGetApiResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.HskGetApiRequest$outboundSchema.parse(value),
  ...
  const pathParams = {
    handle: encodeSimple("handle", payload.handle, {
      explode: false,
  ...
  const path = pathToFunc("/api/{serviceId}/hsk/get/{handle}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.HskGetApiResponse$inboundSchema),
