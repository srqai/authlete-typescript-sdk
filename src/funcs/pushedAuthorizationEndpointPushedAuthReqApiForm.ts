 ...
 */
export function pushedAuthorizationEndpointPushedAuthReqApiForm(
  client: AuthleteCore,
  request: operations.PushedAuthReqApiFormRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.PushedAuthReqApiFormResponse,
    | errors.Error400
    | errors.Error401
    | errors.Error403
    | errors.Error500
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.PushedAuthReqApiFormRequest$outboundSchema.parse(value),
  ...
  const body = Object.entries(payload.api_serviceId_pushed_auth_req || {}).map(
    ([k, v]) => {
  ...
  const path = pathToFunc("/api/{serviceId}/pushed_auth_req")(pathParams);
  ...
  const context = {
    ...
    operationID: "pushed_auth_req_api_form",
    oAuth2Scopes: [],
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.PushedAuthReqApiFormResponse$inboundSchema),
