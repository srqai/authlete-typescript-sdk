 ...
 */
export function verifiableCredentialIssuerVciJwksApiForm(
  client: AuthleteCore,
  request: operations.VciJwksApiFormRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.VciJwksApiFormResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.VciJwksApiFormRequest$outboundSchema.parse(value),
  ...
  const body = Object.entries(payload.APILBraceserviceIdRBraceVciJwks || {})
    .map(([k, v]) => {
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.VciJwksApiFormResponse$inboundSchema),
