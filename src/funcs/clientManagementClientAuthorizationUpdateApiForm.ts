 ...
 */
export function clientManagementClientAuthorizationUpdateApiForm(
  client: AuthleteCore,
  request: operations.ClientAuthorizationUpdateApiFormRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.ClientAuthorizationUpdateApiFormResponse,
    | AuthleteError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.ClientAuthorizationUpdateApiFormRequest$outboundSchema.parse(
        value,
      ),
  ...
  const body = Object.entries(payload.ClientAuthorizationUpdateRequest || {})
    .map(([k, v]) => {
  ...
  const context = {
    ...
    operationID: "client_authorization_update_api_form",
    oAuth2Scopes: [],
  ...
  >(
    M.json(
      200,
      operations.ClientAuthorizationUpdateApiFormResponse$inboundSchema,
    ),
