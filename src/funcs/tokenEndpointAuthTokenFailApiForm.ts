 ...
 * </details>
 */
export function tokenEndpointAuthTokenFailApiForm(
  client: AuthleteCore,
  request: operations.AuthTokenFailApiFormRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.AuthTokenFailApiFormResponse,
    | errors.Error400
    | errors.Error401
    | errors.Error403
    | errors.Error500
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.AuthTokenFailApiFormRequest$outboundSchema.parse(value),
  ...
  const body = Object.entries(payload.api_serviceId_auth_token_fail || {}).map(
    ([k, v]) => {
  ...
  const path = pathToFunc("/api/{serviceId}/auth/token/fail")(pathParams);
  ...
  const context = {
    ...
    operationID: "auth_token_fail_api_form",
    oAuth2Scopes: [],
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.AuthTokenFailApiFormResponse$inboundSchema),
