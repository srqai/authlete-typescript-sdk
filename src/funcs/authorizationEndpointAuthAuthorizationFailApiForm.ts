 ...
 * </details>
 */
export function authorizationEndpointAuthAuthorizationFailApiForm(
  client: AuthleteCore,
  request: operations.AuthAuthorizationFailApiFormRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.AuthAuthorizationFailApiFormResponse,
    | AuthleteError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.AuthAuthorizationFailApiFormRequest$outboundSchema.parse(
        value,
      ),
  ...
  const body = Object.entries(payload.AuthorizationFailRequest || {}).map(
    ([k, v]) => {
  ...
  const path = pathToFunc("/api/{serviceId}/auth/authorization/fail")(
    pathParams,
  );
  ...
  const context = {
    ...
    operationID: "auth_authorization_fail_api_form",
    oAuth2Scopes: [],
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.AuthAuthorizationFailApiFormResponse$inboundSchema),
