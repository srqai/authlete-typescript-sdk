 ...
 * null
 */
export function verifiableCredentialIssuerVciMetadataApiForm(
  client: AuthleteCore,
  request: operations.VciMetadataApiFormRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.VciMetadataApiFormResponse,
    | AuthleteError
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.VciMetadataApiFormRequest$outboundSchema.parse(value),
  ...
  const body = Object.entries(payload.APILBraceserviceIdRBraceVciMetadata || {})
    .map(([k, v]) => {
  ...
  const context = {
    ...
    operationID: "vci_metadata_api_form",
    oAuth2Scopes: [],
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.VciMetadataApiFormResponse$inboundSchema),
