 ...
 * </details>
 */
export function tokenEndpointAuthTokenIssueApiForm(
  client: AuthleteCore,
  request: operations.AuthTokenIssueApiFormRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.AuthTokenIssueApiFormResponse,
    | errors.Error400
    | errors.Error401
    | errors.Error403
    | errors.Error500
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.AuthTokenIssueApiFormRequest$outboundSchema.parse(value),
  ...
  const body = Object.entries(payload.api_serviceId_auth_token_issue || {}).map(
    ([k, v]) => {
  ...
  const path = pathToFunc("/api/{serviceId}/auth/token/issue")(pathParams);
  ...
  const context = {
    ...
    operationID: "auth_token_issue_api_form",
    oAuth2Scopes: [],
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.AuthTokenIssueApiFormResponse$inboundSchema),
