 ...
 * token.
 */
export function tokenEndpointIdtokenReissueApi(
  client: AuthleteCore,
  request: operations.IdtokenReissueApiRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.IdtokenReissueApiResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.IdtokenReissueApiRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/api/{serviceId}/idtoken/reissue")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.IdtokenReissueApiResponse$inboundSchema),
