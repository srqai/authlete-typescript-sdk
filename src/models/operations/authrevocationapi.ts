/*
 * Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
 */

import * as z from "zod";
import { remap as remap$ } from "../../lib/primitives.js";
import { safeParse } from "../../lib/schemas.js";
import { ClosedEnum } from "../../types/enums.js";
import { Result as SafeParseResult } from "../../types/fp.js";
import { SDKValidationError } from "../errors/sdkvalidationerror.js";

export type AuthRevocationApiRequestBody = {
  /**
   * OAuth 2.0 token revocation request parameters which are the request parameters that the OAuth 2.0 token revocation endpoint
   *
   * @remarks
   * ([RFC 7009](https://datatracker.ietf.org/doc/html/rfc7009)) of the authorization server implementation received from the
   * client application.
   *
   * The value of parameters is the entire entity body (which is formatted in `application/x-www-form-urlencoded`) of the request
   * from the client application.
   */
  parameters: string;
  /**
   * The client ID extracted from `Authorization` header of the revocation request from the client application.
   *
   * @remarks
   *
   * If the revocation endpoint of the authorization server implementation supports Basic Authentication
   * as a means of client authentication, and the request from the client application contains its client ID in
   * `Authorization` header, the value should be extracted and set to this parameter.
   */
  clientId?: string | undefined;
  /**
   * The client secret extracted from `Authorization` header of the revocation request from the client application.
   *
   * @remarks
   *
   * If the revocation endpoint of the authorization server implementation supports basic authentication as a means of
   * client authentication, and the request from the client application contained its client secret in `Authorization` header,
   * the value should be extracted and set to this parameter.
   */
  clientSecret?: string | undefined;
  /**
   * The client certificate used in the TLS connection between the client application and the revocation endpoint.
   *
   * @remarks
   */
  clientCertificate?: string | undefined;
  /**
   * The certificate path presented by the client during client authentication.
   *
   * @remarks
   */
  clientCertificatePath?: string | undefined;
};

export type AuthRevocationApiRequest = {
  /**
   * A service ID.
   */
  serviceId: string;
  requestBody: AuthRevocationApiRequestBody;
};

/**
 * The next action that the authorization server implementation should take.
 */
export const AuthRevocationApiAction = {
  InternalServerError: "INTERNAL_SERVER_ERROR",
  InvalidClient: "INVALID_CLIENT",
  BadRequest: "BAD_REQUEST",
  Ok: "OK",
} as const;
/**
 * The next action that the authorization server implementation should take.
 */
export type AuthRevocationApiAction = ClosedEnum<
  typeof AuthRevocationApiAction
>;

/**
 * Successful operation
 */
export type AuthRevocationApiResponse = {
  /**
   * The code which represents the result of the API call.
   */
  resultCode?: string | undefined;
  /**
   * A short message which explains the result of the API call.
   */
  resultMessage?: string | undefined;
  /**
   * The next action that the authorization server implementation should take.
   */
  action?: AuthRevocationApiAction | undefined;
  /**
   * The content that the authorization server implementation is to return to the client application.
   *
   * @remarks
   * Its format varies depending on the value of `action` parameter.
   */
  responseContent?: string | undefined;
};

/** @internal */
export const AuthRevocationApiRequestBody$inboundSchema: z.ZodType<
  AuthRevocationApiRequestBody,
  z.ZodTypeDef,
  unknown
> = z.object({
  parameters: z.string(),
  clientId: z.string().optional(),
  clientSecret: z.string().optional(),
  clientCertificate: z.string().optional(),
  clientCertificatePath: z.string().optional(),
});

/** @internal */
export type AuthRevocationApiRequestBody$Outbound = {
  parameters: string;
  clientId?: string | undefined;
  clientSecret?: string | undefined;
  clientCertificate?: string | undefined;
  clientCertificatePath?: string | undefined;
};

/** @internal */
export const AuthRevocationApiRequestBody$outboundSchema: z.ZodType<
  AuthRevocationApiRequestBody$Outbound,
  z.ZodTypeDef,
  AuthRevocationApiRequestBody
> = z.object({
  parameters: z.string(),
  clientId: z.string().optional(),
  clientSecret: z.string().optional(),
  clientCertificate: z.string().optional(),
  clientCertificatePath: z.string().optional(),
});

/**
 * @internal
 * @deprecated This namespace will be removed in future versions. Use schemas and types that are exported directly from this module.
 */
export namespace AuthRevocationApiRequestBody$ {
  /** @deprecated use `AuthRevocationApiRequestBody$inboundSchema` instead. */
  export const inboundSchema = AuthRevocationApiRequestBody$inboundSchema;
  /** @deprecated use `AuthRevocationApiRequestBody$outboundSchema` instead. */
  export const outboundSchema = AuthRevocationApiRequestBody$outboundSchema;
  /** @deprecated use `AuthRevocationApiRequestBody$Outbound` instead. */
  export type Outbound = AuthRevocationApiRequestBody$Outbound;
}

export function authRevocationApiRequestBodyToJSON(
  authRevocationApiRequestBody: AuthRevocationApiRequestBody,
): string {
  return JSON.stringify(
    AuthRevocationApiRequestBody$outboundSchema.parse(
      authRevocationApiRequestBody,
    ),
  );
}

export function authRevocationApiRequestBodyFromJSON(
  jsonString: string,
): SafeParseResult<AuthRevocationApiRequestBody, SDKValidationError> {
  return safeParse(
    jsonString,
    (x) => AuthRevocationApiRequestBody$inboundSchema.parse(JSON.parse(x)),
    `Failed to parse 'AuthRevocationApiRequestBody' from JSON`,
  );
}

/** @internal */
export const AuthRevocationApiRequest$inboundSchema: z.ZodType<
  AuthRevocationApiRequest,
  z.ZodTypeDef,
  unknown
> = z.object({
  serviceId: z.string(),
  RequestBody: z.lazy(() => AuthRevocationApiRequestBody$inboundSchema),
}).transform((v) => {
  return remap$(v, {
    "RequestBody": "requestBody",
  });
});

/** @internal */
export type AuthRevocationApiRequest$Outbound = {
  serviceId: string;
  RequestBody: AuthRevocationApiRequestBody$Outbound;
};

/** @internal */
export const AuthRevocationApiRequest$outboundSchema: z.ZodType<
  AuthRevocationApiRequest$Outbound,
  z.ZodTypeDef,
  AuthRevocationApiRequest
> = z.object({
  serviceId: z.string(),
  requestBody: z.lazy(() => AuthRevocationApiRequestBody$outboundSchema),
}).transform((v) => {
  return remap$(v, {
    requestBody: "RequestBody",
  });
});

/**
 * @internal
 * @deprecated This namespace will be removed in future versions. Use schemas and types that are exported directly from this module.
 */
export namespace AuthRevocationApiRequest$ {
  /** @deprecated use `AuthRevocationApiRequest$inboundSchema` instead. */
  export const inboundSchema = AuthRevocationApiRequest$inboundSchema;
  /** @deprecated use `AuthRevocationApiRequest$outboundSchema` instead. */
  export const outboundSchema = AuthRevocationApiRequest$outboundSchema;
  /** @deprecated use `AuthRevocationApiRequest$Outbound` instead. */
  export type Outbound = AuthRevocationApiRequest$Outbound;
}

export function authRevocationApiRequestToJSON(
  authRevocationApiRequest: AuthRevocationApiRequest,
): string {
  return JSON.stringify(
    AuthRevocationApiRequest$outboundSchema.parse(authRevocationApiRequest),
  );
}

export function authRevocationApiRequestFromJSON(
  jsonString: string,
): SafeParseResult<AuthRevocationApiRequest, SDKValidationError> {
  return safeParse(
    jsonString,
    (x) => AuthRevocationApiRequest$inboundSchema.parse(JSON.parse(x)),
    `Failed to parse 'AuthRevocationApiRequest' from JSON`,
  );
}

/** @internal */
export const AuthRevocationApiAction$inboundSchema: z.ZodNativeEnum<
  typeof AuthRevocationApiAction
> = z.nativeEnum(AuthRevocationApiAction);

/** @internal */
export const AuthRevocationApiAction$outboundSchema: z.ZodNativeEnum<
  typeof AuthRevocationApiAction
> = AuthRevocationApiAction$inboundSchema;

/**
 * @internal
 * @deprecated This namespace will be removed in future versions. Use schemas and types that are exported directly from this module.
 */
export namespace AuthRevocationApiAction$ {
  /** @deprecated use `AuthRevocationApiAction$inboundSchema` instead. */
  export const inboundSchema = AuthRevocationApiAction$inboundSchema;
  /** @deprecated use `AuthRevocationApiAction$outboundSchema` instead. */
  export const outboundSchema = AuthRevocationApiAction$outboundSchema;
}

/** @internal */
export const AuthRevocationApiResponse$inboundSchema: z.ZodType<
  AuthRevocationApiResponse,
  z.ZodTypeDef,
  unknown
> = z.object({
  resultCode: z.string().optional(),
  resultMessage: z.string().optional(),
  action: AuthRevocationApiAction$inboundSchema.optional(),
  responseContent: z.string().optional(),
});

/** @internal */
export type AuthRevocationApiResponse$Outbound = {
  resultCode?: string | undefined;
  resultMessage?: string | undefined;
  action?: string | undefined;
  responseContent?: string | undefined;
};

/** @internal */
export const AuthRevocationApiResponse$outboundSchema: z.ZodType<
  AuthRevocationApiResponse$Outbound,
  z.ZodTypeDef,
  AuthRevocationApiResponse
> = z.object({
  resultCode: z.string().optional(),
  resultMessage: z.string().optional(),
  action: AuthRevocationApiAction$outboundSchema.optional(),
  responseContent: z.string().optional(),
});

/**
 * @internal
 * @deprecated This namespace will be removed in future versions. Use schemas and types that are exported directly from this module.
 */
export namespace AuthRevocationApiResponse$ {
  /** @deprecated use `AuthRevocationApiResponse$inboundSchema` instead. */
  export const inboundSchema = AuthRevocationApiResponse$inboundSchema;
  /** @deprecated use `AuthRevocationApiResponse$outboundSchema` instead. */
  export const outboundSchema = AuthRevocationApiResponse$outboundSchema;
  /** @deprecated use `AuthRevocationApiResponse$Outbound` instead. */
  export type Outbound = AuthRevocationApiResponse$Outbound;
}

export function authRevocationApiResponseToJSON(
  authRevocationApiResponse: AuthRevocationApiResponse,
): string {
  return JSON.stringify(
    AuthRevocationApiResponse$outboundSchema.parse(authRevocationApiResponse),
  );
}

export function authRevocationApiResponseFromJSON(
  jsonString: string,
): SafeParseResult<AuthRevocationApiResponse, SDKValidationError> {
  return safeParse(
    jsonString,
    (x) => AuthRevocationApiResponse$inboundSchema.parse(JSON.parse(x)),
    `Failed to parse 'AuthRevocationApiResponse' from JSON`,
  );
}
